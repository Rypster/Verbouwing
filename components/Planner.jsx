...
export default function Planner() {
  ...
  // Cloud / delen
  const [cloudProject, setCloudProject] = useState(null); // { id, token } | null
  ...
  const cloudRef = useRef(cloudProject);
  cloudRef.current = cloudProject;
  ...
  function scheduleSave() {
    ...
    saveTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        // En naar cloud als we een gedeeld project hebben
        const cloud = cloudRef.current;
        if (cloud?.id && cloud?.token) {
          const res = await fetch('/api/project', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: cloud.id, token: cloud.token, data: snapshot })
          });
          if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            throw new Error(err.error || 'Cloud opslaan mislukt');
          }
        }
  ...
  async function createShareLink() {
    ...
    try {
      ...
      const url = `${window.location.origin}${window.location.pathname}?p=${cloud.id}&t=${cloud.token}`;
      ...
      console.error(e);
      setShareMsg(e.message || 'Mislukt – is DATABASE_URL gezet?');
      ...
      try {
        ...
        if (cloud?.id && cloud?.token) {
          const res = await fetch('/api/project', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: cloud.id, token: cloud.token, data: loaded })
          });
          if (!res.ok) console.warn('Cloud update na restore mislukt');
        }
        setShareMsg('Backup hersteld');
        setTimeout(() => setShareMsg(''), 2500);
        scheduleSave();
      } catch (err) {
        console.error(err);
  ...
  // Laden: URL-params (cloud) → anders lokaal IndexedDB
  useEffect(() => {
    // Sessie-id voor presence
    ...
    async function load() {
      ...
      if (p && t) {
        try {
          setSaveState('laden…');
          const res = await fetch(`/api/project?id=${encodeURIComponent(p)}&token=${encodeURIComponent(t)}`);
          if (res.ok) {
            const json = await res.json();
            const loaded = normalizeLoaded(json.data || {});
            setState(loaded);
            (loaded.backgrounds || []).forEach(buildBgPixelCache);
            const cloud = { id: p, token: t };
            setCloudProject(cloud);
            localStorage.setItem('vp_cloud', JSON.stringify(cloud));
            await idbSet('project', JSON.stringify(loaded));
            setSaveState('opgeslagen');
          ...
          console.error('Cloud laden mislukt', e);
        }
      }

      // Fallback: eerder opgeslagen cloud-id + lokaal
      try {
        const stored = localStorage.getItem('vp_cloud');
        if (stored) {
          const cloud = JSON.parse(stored);
          if (cloud?.id && cloud?.token) setCloudProject(cloud);
        }
      } catch (_) {}
  ...
  // Presence heartbeat (alleen bij gedeeld project)
  useEffect(() => {
    if (!cloudProject?.id || !sessionIdRef.current) return;
    ...
    async function beat() {
      try {
        ...
        if (res.ok && !stopped) {
          const json = await res.json();
          setOthersOnline(json.othersOnline || 0);
        }
      } catch (_) {}
    }
  ...
  return (
    <div className="vp-root">
      ...
      <div className="vp-toolbar">
        ...
        <button className="vp-btn" onClick={copyShareLink} title="Maak of kopieer deellink (lees+schrijf)">
          🔗 Deellink
        </button>
        {shareMsg && <span className="vp-label" style={{ color: '#7dd3fc' }}>{shareMsg}</span>}
        {/* Online-lampje: brandt als iemand anders dit project open heeft */}
        <span
          title={
            !cloudProject
              ? 'Nog geen deellink – maak er een om samen te werken'
              : othersOnline > 0
                ? `${othersOnline} ander(en) online`
                : 'Niemand anders online'
          }
          style={{
            display: 'inline-flex',
            alignItems: 'center',
        ...
        >
          <span
            style={{
              ...
              height: 10,
              borderRadius: '50%',
              background: !cloudProject
                ? '#4b5563'
                : othersOnline > 0
                  ? '#22c55e'
                  : '#6b7280',
              boxShadow: othersOnline > 0 ? '0 0 8px #22c55e' : 'none'
            }}
          />
          {cloudProject
            ? (othersOnline > 0 ? `${othersOnline} online` : 'alleen jij')
            : 'lokaal'}
        </span>
